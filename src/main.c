...
// THIS PROGRAM IS MEANT FOR UNIX-LIKE SYSTEMS SYSTEMS
...
int execute(char *command, char **argument) {
  if (!command || !argument || !argument[0]) {
...
char** tokenize(char *command, const char *delim) {
  ...
  // Handle cases where arguments exceed capacity
  while (token != NULL) {
...
int main(int argc, char *argv[]) {
  ...
  while (1) {
    ...
    // Initialized reusable variables for command processing
    ...
    if (strcmp(argument[0], "type") == 0) {
      ...
      if (argument[1] && (strcmp(argument[1], "echo") == 0 || strcmp(argument[1], "exit") == 0 || strcmp(argument[1], "type" ) == 0 ||strcmp(argument[1], "pwd" ) == 0)) {
